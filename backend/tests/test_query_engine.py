...
Expected values are computed independently against the same DuckDB connection
...
def _scalar(engine, sql: str, params: dict | None = None):
    row = engine._con.execute(sql, params or {}).fetchone()
...
# ---------------------------------------------------------------------------
# Aggregation correctness against DuckDB ground truth
# ---------------------------------------------------------------------------
...
def test_debit_sum_aug_2026(duck_engine):
    ...
    expected = _scalar(
        ...
        WHERE t.transaction_type = 'debit'
          AND CAST(t.transaction_date AS DATE) >= DATE '2026-08-01'
          AND CAST(t.transaction_date AS DATE) <= DATE '2026-08-31'
        """,
    )
    ...
    assert result.query_metadata["backend"] == "duckdb"
...
def test_list_records_count_exceeds_shown(duck_engine):
    ...
    expected_all = _scalar(
        ...
        """
        SELECT COUNT(DISTINCT transaction_id) FROM "transaction"
        WHERE CAST(transaction_date AS DATE) >= DATE '2026-08-01'
          AND CAST(transaction_date AS DATE) <= DATE '2026-08-31'
        """,
    )
...
def test_monthly_trend_peak(duck_engine):
    ...
    rows = duck_engine._con.execute(
        """
        SELECT strftime(transaction_date, '%Y-%m'), SUM(transaction_amount)
        FROM "transaction"
        GROUP BY 1
        """
    ).fetchall()
...
def test_comparison_jul_aug_2026(duck_engine):
    ...
    expected_jul = _scalar(
        ...
        FROM "transaction" t
        WHERE t.transaction_type = 'debit'
          AND CAST(t.transaction_date AS DATE) >= DATE '2026-07-01'
          AND CAST(t.transaction_date AS DATE) <= DATE '2026-07-31'
        """,
    )
