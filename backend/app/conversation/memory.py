...
Stores STRUCTURED context (last intent, metric, filters, date range) — not the
raw transcript. Follow-ups like "how does that compare with the month before?"
resolve against this explicit state.
"""
...
class ConversationContext:
    ...
    def apply(self, q: FinancialQuery, answer_value) -> None:
        self.last_intent = q.intent.value
        self.last_metric = q.metric.value
        self.last_vendor = q.filters.vendor_name or q.filters.vendor_id or self.last_vendor
...
class ConversationStore:
    """In-memory session store keyed by conversation id. Swap for Redis/Postgres
